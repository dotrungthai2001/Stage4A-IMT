...
# Define a function to generate noisy images on-the-fly
def generate_noisy_images(batch_size):
    noisy_images = []
    for _ in range(batch_size):
        # Generate a clean image
        n = np.random.randint(5, 8)
        n_pts = np.random.randint(100, 200)
        x = []
        y = []
        ...
        noisy_image = add_gaussian_noise(np.array(clean_image), noise_std)
        noisy_image = torch.FloatTensor(noisy_image) / 255.0  # Normalize noisy image
        clean_image = torch.FloatTensor(clean_image) / 255.0  # Normalize clean image
...
def main(args):
	...
	for epoch in range(start_epoch, args.epochs):        
		...
		for i, data in enumerate(loader_train, 0):
            ...
            img_train = data
            noise = generate_noisy_images(args.batch_size)  # Generate noisy images on-the-fly
